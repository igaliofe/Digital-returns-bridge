...
function runDevScript(task: 'nuke' | 'up'): void {
...
/**
 * database/seed.sql only runs on a FRESH postgres volume (it is mounted into
 * docker-entrypoint-initdb.d), so a plain restart keeps whatever junk a previous
 * run left behind. `nuke` drops the volumes; `up` recreates them and re-runs
